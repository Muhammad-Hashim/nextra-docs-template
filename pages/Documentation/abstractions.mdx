...
This documentation will guide you through the evolution of tinygrad, taking you on a journey from a Tensor to the underlying details. Over the 2.5 years of development, tinygrad has undergone aggressive refactoring, resulting in a refined library that continues to evolve.

The entire tinygrad codebase is approximately 2300 lines, making it readable in an evening or two. This documentation aims to provide insights into entry points and help you understand the abstraction stack.\
Vist here to check [abstractions.py](https://github.com/tinygrad/tinygrad/blob/master/docs/abstractions.py)
...
class Tensor:
  ...
  # high level ops (hlops) are defined on this class. Ex: relu
...
``` python
class Function:
  # Ex: types of forward and backward
...
Fom tinygrad.helpers import DType\
...
NOTE: if you have a CompiledBuffer(DeviceBuffer) you do not need to implement the MovementOps   as they are handled by the ShapeTracker(in tinygrad/shape/shapetracker.py, code 7/10)
...
Interpreted backends are very simple (Ex: CPU and TORCH)
``` python
class Interpreted:
...
# Compiled backends take a little more (Ex: GPU and LLVM)
class Compiled:
...
``` 
 RawNumpyBuffer is a RawBuffer Ex: for numpy. It's very simple
``` python
...
it's just malloc(size * dtype.itemsize) from tinygrad.runtime.lib import RawMallocBuffer\
...
# a concrete Ex: looks like this, this adds two size 1 RawBuffer
...
in the above Ex:, we wrote the code by hand\
...
####  Ex: 2+3 autogenerated clang code 
to generate clang code, the Linearizer is wrapped with CStyleCodegen\
 here, we have an Ex: where we fetch the generated code from the JIT
...
#### Ex: ShapeTracker (in tinygrad/shape/shapetracker.py, code 7/10) ==
...
# %%
# == Ex: Variable (in tinygrad/shape/symbolic.py, code 6/10) ==
